...
<body class="p-4"
	...
	<div class="container-fluid p-4">
		<h2 class="text-center text-light font-weight-bold mt-4">Timetable List</h2>
		...
		<form action="<%=ORSView.TIMETABLE_LIST_CTL%>" method="post">
                    ...
                    <tr>
                        <td>
                            <div class="d-flex justify-content-center align-items-center flex-wrap bg-light bg-opacity-75 p-3 rounded shadow-sm">
                                ...
                                <div class="mx-2">
                                    <input type="date" class="form-control form-control-sm" id="udatee" name="examDate"
                                        value="<%=DataUtility.getDateString(dto.getExamDate())%>">
                                </div>
                                <div class="mx-2">
                                    <input type="submit" class="btn btn-sm btn-primary" name="operation" value="<%=TimeTableListCtl.OP_SEARCH%>">
                                    <input type="submit" class="btn btn-sm btn-outline-secondary ml-1" name="operation" value="<%=TimeTableListCtl.OP_RESET%>">
                                </div>
                            </div>
                        </td>
                    </tr>
			...
			<table class="table w-100">
				<tr>
					<td width="25%"><input type="submit" class="btn btn-outline-primary" name="operation" value="<%=TimeTableListCtl.OP_PREVIOUS%>" <%=pageNo > 1 ? "" : "disabled"%>></td>
					<td width="25%" class="text-center"><input type="submit" class="btn btn-outline-success" name="operation" value="<%=TimeTableListCtl.OP_NEW%>"></td>
					<td width="25%" class="text-center"><input type="submit" class="btn btn-outline-danger" name="operation" value="<%=TimeTableListCtl.OP_DELETE%>"></td>
					<td width="25%" class="text-right"><input type="submit" class="btn btn-outline-primary" name="operation" value="<%=TimeTableListCtl.OP_NEXT%>" <%= (nextPageSize != 0) ? "" : "disabled"%>></td>
				</tr>
			</table>
			...
			<table class="table w-100">
				<tr>
					<td class="text-right"><input type="submit" class="btn btn-outline-secondary" name="operation" value="<%=TimeTableListCtl.OP_BACK%>"></td>
				</tr>
			</table>
			<%
				}
