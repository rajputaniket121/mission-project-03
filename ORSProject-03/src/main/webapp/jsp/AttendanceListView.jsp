<%@page import="in.co.rays.proj3.controller.AttendanceListCtl"%>
<%@page import="in.co.rays.proj3.utill.HTMLUtility"%>
<%@page import="in.co.rays.proj3.utill.DataUtility"%>
<%@page import="in.co.rays.proj3.dto.AttendanceDTO"%>
    ...
    %>
        ...
        <form action="<%=ORSView.ATTENDANCE_LIST_CTL%>" method="post">
            ...
            List<AttendanceDTO> list =
                (List<AttendanceDTO>) ServletUtility.getList(request);
                            ...
                            <div
                                ...
                                <div class="mx-2">
                                    <input type="text" class="form-control form-control-sm"
                                        name="personName" placeholder="Enter Person Name"
                                        value="<%=ServletUtility.getParameter("personName", request)%>">
                                </div>
                                <div class="mx-2">
                                    <label><b>Attendance Status :</b></label>
                                </div>
                                <div class="mx-2">
                                    <%=HTMLUtility.getList("attendanceStatus", String.valueOf(dto.getAttendanceStatus()), attendanceStatusMap)%>
                                </div>
                                <div class="mx-2">
                                    <input type="submit" class="btn btn-sm btn-primary"
                                        name="operation" value="<%=AttendanceListCtl.OP_SEARCH%>">
                                    <input type="submit"
            ...
            <div class="table-responsive">
                <table
                    ...
                    <tbody>
                        <%
                    while (it.hasNext()) {
                        dto = (AttendanceDTO) it.next();
                %>
                        <tr>
                            <td><input type="checkbox" class="case" name="ids"
                                value="<%=dto.getId()%>"></td>
                            <td><%=index++%></td>
                            <td class="text-capitalize"><%=dto.getPersonName()%></td>
                            <td><%=DataUtility.getDateString(dto.getAttendanceDate())%></td>
                            <td><%=dto.getAttendanceStatus()%></td>
                            <td><%=dto.getRemarks()%></td>
                            <td><a href="AttendanceCtl?id=<%=dto.getId()%>"
            ...
            <table class="table w-100">
                <tr>
                    ...
                    <td width="25%" class="text-center"><input type="submit"
                        ...
                        value="<%=AttendanceListCtl.OP_NEW%>"></td>

                    <td width="25%" class="text-center"><input type="submit"
                        class="btn btn-outline-danger" name="operation"
                        value="<%=AttendanceListCtl.OP_DELETE%>"></td>
