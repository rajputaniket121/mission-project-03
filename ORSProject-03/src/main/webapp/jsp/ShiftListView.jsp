<%@page import="in.co.rays.proj3.controller.ShiftListCtl"%>
<%@page import="in.co.rays.proj3.utill.HTMLUtility"%>
<%@page import="in.co.rays.proj3.utill.DataUtility"%>
<%@page import="in.co.rays.proj3.dto.ShiftDTO"%>
...
<body class="p4">
    ...
    %>
        ...
        <form action="<%=ORSView.SHIFT_LIST_CTL%>" method="post">
            ...
            List<ShiftDTO> list =
                (List<ShiftDTO>) ServletUtility.getList(request);
                            ...
                            <div
                                ...
                                <div class="mx-2">
                                    <input type="text" class="form-control form-control-sm"
                                        name="shiftCode" placeholder="Enter Shift Code"
                                ...
                                <div class="mx-2">
                                    <input type="text" class="form-control form-control-sm"
                                        name="shiftName" placeholder="Enter Shift Name"
                                        value="<%=ServletUtility.getParameter("shiftName", request)%>">
                                </div>
                                <div class="mx-2">
                                    <label><b>Shift Status :</b></label>
                                </div>
                                <div class="mx-2">
                                    <%=HTMLUtility.getList("shiftStatus", String.valueOf(dto.getShiftStatus()), shiftStatusMap)%>
                                </div>
                                <div class="mx-2">
                                    <input type="submit" class="btn btn-sm btn-primary"
                                        name="operation" value="<%=ShiftListCtl.OP_SEARCH%>">
                                    <input type="submit"
            ...
            <div class="table-responsive">
                <table
                    ...
                    <tbody>
                        <%
                    while (it.hasNext()) {
                        dto = (ShiftDTO) it.next();
                %>
                        <tr>
                            <td><input type="checkbox" class="case" name="ids"
                                value="<%=dto.getId()%>"></td>
                            <td><%=index++%></td>
                            <td><%=dto.getShiftCode()%></td>
                            <td><%=dto.getShiftName()%></td>
                            <td><%=DataUtility.getDateString(dto.getStartTime())%></td>
                            <td><%=DataUtility.getDateString(dto.getEndTime())%></td>
                            <td><%=dto.getShiftStatus()%></td>
                            <td><a href="ShiftCtl?id=<%=dto.getId()%>"
            ...
            <table class="table w-100">
                <tr>
                    ...
                    <td width="25%" class="text-center"><input type="submit"
                        ...
                        value="<%=ShiftListCtl.OP_NEW%>"></td>

                    <td width="25%" class="text-center"><input type="submit"
                        class="btn btn-outline-danger" name="operation"
                        value="<%=ShiftListCtl.OP_DELETE%>"></td>
